    ...
    <section className="bg-transparent dark:bg-card text-foreground transition-colors duration-300 relative overflow-hidden min-h-screen pt-24 pb-20">
      <div className="flex flex-col items-center justify-center text-center px-4 mb-20 relative z-10 w-full max-w-5xl mx-auto">
        ...
        <motion.div 
          ...
          className="w-full max-w-2xl h-1.5 bg-gradient-to-r from-transparent via-primary/50 to-transparent rounded-full mb-8 shadow-[0_0_15px_rgb(var(--primary)/0.3)]"
        />
        ...
        >
          {allinfo.map((item, index) => (
            <motion.div
              ...
              key={index}
              className="imp-hoverable group relative w-full h-auto min-h-[450px] rounded-[2.5rem] overflow-hidden cursor-pointer shadow-sm dark:shadow-black/50 border border-stone-200/60 dark:border-white/10 bg-white/60 dark:bg-black/30 backdrop-blur-md flex flex-col"
            >
              {/* Image Container */}
              ...
              <div className="w-full flex-grow flex flex-col items-center p-6 text-center z-20 pt-2">
                <div className="w-12 h-1.5 bg-gradient-to-r from-primary to-secondary rounded-full mb-4 opacity-50 group-hover:opacity-100 transition-opacity duration-500 shadow-[0_0_10px_rgb(var(--primary)/0.5)]" />
